...
Additionally for train and val data:
...
tf.app.flags.DEFINE_string('custom_root',
                           './custom',
...
def _get_files(data, dataset_split):
  ...
  if dataset_split == 'test':
    pattern = '*.%s' % (_TEST_FORMAT_MAP[data])
  else:
...
def _convert_dataset(dataset_split):
  ...
  if dataset_split == 'test':
    image_reader = build_data.ImageReader(_TEST_FORMAT_MAP['image'], channels=3)
  else:
    image_reader = build_data.ImageReader(_DATA_FORMAT_MAP['image'], channels=3)
    label_reader = build_data.ImageReader(_DATA_FORMAT_MAP['label'], channels=1)
    ...
    with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
      ...
      for i in range(start_idx, end_idx):
        ...
        if dataset_split!='test':
          ...
          example = build_data.image_to_tfexample(image_data, filename, height, width)
